function TaskCard({allTasks, setAllTasks, tasks, title}) {
  const updateField = (fieldValue, fieldName, taskId) => {
      let updatedField = { };
        const updatedTasks = allTasks.map((task) => {
          if (task.id === taskId) {
                updatedField[fieldName] = fieldValue;
                return { ...task, ...updatedField, updatedDate: new Date().toISOString()};
            }
            return task;
        });
  ...
  return (
    <div className="mb-8">
        ...
        <ul className="list-disc pl-5">
            {tasks.map((task) => (
                <li key={task.id} className="py-1 flex justify-between items-center">
                    ...
                    <span className="mr-3 text-gray-400">
                        ...
                        <select
                            value={task.priority}
                            onChange={(e) =>
                                updateField(e.target.value, 'priority', task.id)
                            }
                    ...
                    <select
                        value={task.status}
                        onChange={(e) =>
                            updateField(e.target.value, 'status', task.id)
                        }
                        className="ml-2 bg-gray-600 text-gray-100 rounded px-1 py-1 cursor-pointer hover:opacity-85"
